...
fontN='Calibri';
s = get(0, 'ScreenSize');
figA=figure('Position', [0 0 s(3)/1.2 s(4)]);
tic
%load Bursts%%
load Bursts8
...
dz=dx;%
x=0:dx:10; %m  Her velger du størrelsen på området!!
z=0:dz: 8;% m
...
for n=1:N
    for m=1:M
        K=B/r1(n,m);%Merk: ingen forsinkelse av første bidrag!
        H=K0;
        H(DT(n,m)+1:nS)=B(1:nS-DT(n,m)); %Bidrag nr 2
        K=K+R2*H/r2(n,m);
        JJ(n,m,:)=K(1:nS);%% JJ inneholder hele signalet
        %gjer JJ om til sparce matrix, lag K0 sparce først 
...
figure(1)
[c,H]=contour(x,-z,P1',0:500:7000,'r');
